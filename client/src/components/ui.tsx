...
export function Card({
  ...
  return (
    <div
      className={cn(
        "prime-card rounded-2xl border border-ink-200/80 bg-white/90 shadow-[0_1px_2px_rgba(15,23,42,0.03),0_12px_32px_-18px_rgba(15,23,42,0.18)] backdrop-blur-sm",
        className,
      )}
    >
      {children}
...
export function Label({ children, className, htmlFor, ...props }: React.LabelHTMLAttributes<HTMLLabelElement>) {
  ...
  useEffect(() => {
    ...
    const control = ref.current.parentElement?.querySelector<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>("input, select, textarea");
    if (control && !control.id) control.id = generatedId;
  }, [generatedId, htmlFor]);
...
export function Badge({
  ...
  return (
    <span
      className={cn(
        "badge inline-flex min-h-7 shrink-0 whitespace-nowrap items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold leading-none capitalize ring-1 ring-inset",
        className,
      )}
    >
      {children}
...
export function SeverityBadge({ value }: { value: string }) {
  return (
    <Badge className={cn("severity-badge", SEVERITY_STYLES[value] ?? SEVERITY_STYLES.medium)}>
      <span
...
export function StatusBadge({ value }: { value: string }) {
  return (
    <Badge className={cn("status-badge", STATUS_STYLES[value] ?? STATUS_STYLES.open)}>
...
export function SlaBadge({
  ...
  return (
    <Badge className={cn("sla-badge", TONE_STYLES[sla.tone ?? "neutral"], "normal-case", className)}>
          <span className="status-dot" aria-hidden="true" />
          {sla.label}
        </Badge>
  );
...
export function Tone({
  ...
  return <Badge className={cn(TONE_STYLES[tone], "normal-case", className)}>{children}</Badge>;
}
