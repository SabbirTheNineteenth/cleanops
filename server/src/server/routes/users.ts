...
const activeAdminDemotionGuard = sql`(
  select count(*) from ${users}
  where ${users.role} = 'admin' and ${users.status} = 'active'
) > 1`;
...
userRoutes.patch("/:id", async (c) => {
  ...
  const isActiveAdminDemotion = target.role === "admin" && target.status === "active" && role === "user";
  const row = await db
    .update(users)
    .set(patch)
    .where(isActiveAdminDemotion ? and(eq(users.id, id), activeAdminDemotionGuard) : eq(users.id, id))
    .returning(publicUserCols)
    .get();
  if (!row) return c.json({ error: "Keep at least one active admin" }, 400);
